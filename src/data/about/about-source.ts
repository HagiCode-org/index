...
type AboutLinkEntry = AboutBaseEntry & {
  readonly type: 'link';
  readonly url: string;
};

type AboutContactEntry = AboutBaseEntry & {
  readonly type: 'contact';
  readonly value: string;
  readonly url?: string;
};

type AboutImageEntry = AboutBaseEntry & {
  readonly type: 'qr' | 'image';
  readonly imageId: AboutImageId;
  readonly alt: string;
  readonly url?: string;
};

export type AboutSourceEntry = AboutLinkEntry | AboutContactEntry | AboutImageEntry;

export interface AboutSourceDocument {
  readonly version: string;
  readonly updatedAt: string;
  readonly entries: readonly AboutSourceEntry[];
}
...
} as const satisfies AboutSourceDocument;
